...
layout( constant_id = 5 ) const uint NUM_Z_TILES = 64;
layout( constant_id = 6 ) const uint ELEMENTS_PER_CLUSTER = 2;
layout( constant_id = 7 ) const uint MODE = 0;
...
//const uint NUM_Z_TILES = 64;
//const uint ELEMENTS_PER_CLUSTER = 2;
//const uint MODE = 0;

layout(set = 2, binding = 1) uniform BoundUniform
{
...
void main(){
	ivec2 tilePosXY = ivec2(gl_FragCoord.xy);
	...
	{
		ivec3 tileCoords = ivec3(tilePosXY, zTile);
		...
		if(MODE == 2 && (cluster.data[address * 4] & mask) != 0)break;
		cluster.data[address * 4] |= mask;
	}
}